...
## 何をするか
複数のパッチを `classes4.dex` のみ差し替えで適用する。各パッチはアプリ内設定画面から個別に ON/OFF でき、既定は全て有効（裏機能の「時間外チェックイン」のみ既定オフ）。
...
公式は受付時間外（`MenuForDayDTO.isCheckInTime()==false`）になるとホームの「チェックイン」ボタンを
グレーアウト＋無効化する。本パッチは ON のとき、未チェックインの時間外ボタンをグレー見た目のまま
**押下可能**にし、タップ時にアプリ純正の確認ダイアログ（予約食事名＝和食/洋食/夕食を明記）を表示。
OK で公式の `CheckInDialog` を開く（和食/洋食の正しい表示と実チェックインは公式ロジックを再利用）。
送信は公式と同一の `POST v1/reservations/checkin`（`reservation_id` のみ）で、独自通信は行わない。
**裏機能のため既定オフ**。サーバーが時間窓を強制していれば時間外要求は弾かれる（詳細は patch-notes）。
