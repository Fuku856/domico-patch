...
- 自動チェックイン通知の小アイコンに公式の ic_push_notification を使う
- 自動チェックイン通知の確認待ちが残る/背面復帰で誤判定する問題を修正

### Features

- 自動チェックインの送信と結果をシステム通知で伝える

## [0.8.2] - 2026-09-18

### Bug Fixes

- 時間外チェックインの切り替えを子行の表示へ即時反映する
- モジュール dex から smali クラスへの参照も検証対象に含める

### CI

- Setup-android が廃止済み 'tools' パッケージの導入で失敗する問題を修正
- Dry-run と実ビルドで同じ build-tools を使うよう固定
- Dry-run の行継続が壊れて --check に不正な引数が渡る問題を修正

### Features

- 受信push全ペイロードを端末内ファイルへ保存するデバッグパッチを追加
- パッチ本体を別dexモジュール化する土台を追加
- 純粋な9クラスを Java モジュールへ移行し、参照検証を追加

### Other

- Main を dev に取り込み (PR #35 のコンフリクト解消)
